...
#SiteInstallJob: batchv1.#Job & {
	...
	kind:       "Job"
	metadata: {
		name:      "\(#config.metadata.name)-site-install"
		namespace: #config.metadata.namespace
		labels:    #config.metadata.labels
	}
	spec: batchv1.#JobSpec & {
		backoffLimit:             #config.drupal.backoffLimitInstall
				...
				containers: [
					{
						...
						command: [
							...
							if [ "true" = "true" ]; then
							    # Ensure MySQL client has the password for the connection check if needed.
							    if [ "\#(#config.mysql.enabled)" = "true" ]; then
							        export MYSQL_PWD=$MYSQL_PASSWORD
							    fi

							    if [ "\#(#config.drupal.install)" = "true" ]; then
							        echo "Running drush site:install..."
							        time drush site:install \#(#config.drupal.profile) \
							          --sites-subdir=default \
							          \#([
	                                     if #config.external.enabled { "--db-url=\"\(#config.external.driver)://\(#config.external.user):$EXTERNAL_PASSWORD@\(#config.external.host):\(#config.external.port)/\(#config.external.database)\" \\" },
	                                     if #config.mysql.enabled { "--db-url=\"mysql://\(#config.mysql.auth.username):$MYSQL_PASSWORD@\(#config.metadata.name)-mysql:3306/\(#config.mysql.auth.database)\" \\" },
	                                     if #config.postgresql.enabled { "--db-url=\"pgsql://\(#config.postgresql.auth.username):$POSTGRES_PASSWORD@\(#config.metadata.name)-postgresql:5432/\(#config.postgresql.auth.database)\" \\" },
	                                     ""
	                                  ][0])
							          --account-name=\#(#config.drupal.username) \
							          --account-pass=$DRUPAL_ADMIN_PASSWORD \
							          --site-mail="\#(#config.drupal.siteEmail)" \
							          --site-name="\#(#config.drupal.siteName)" \
							          wxt_extension_configure_form.select_all='TRUE' \
							          install_configure_form.update_status_module='array(FALSE,FALSE)' \
							          --yes
							    else
							        echo "drupal.install is false, skipping site-install."
							    fi
							fi
							...
							\#(#config.drupal.postInstallScripts)
							"""#
							,
						]
