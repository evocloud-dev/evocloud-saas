...
#SchedulerDeployment: appsv1.#Deployment & {
	...
	metadata: {
		...
		labels: {
			...
			"app.kubernetes.io/name":     "\(#config.metadata.name)-scheduler"
			"app.kubernetes.io/instance": #config.metadata.name
			"app.kubernetes.io/app":      "frappe"
		}
	}
	spec: appsv1.#DeploymentSpec & {
		replicas: #config.worker.scheduler.replicaCount
		...
		template: {
			...
			spec: corev1.#PodSpec & {
				...
				if #config.worker.scheduler.initContainers != _|_ {
					initContainers: #config.worker.scheduler.initContainers
				}
				containers: [
					{
						name:            "scheduler"
						...
						if #config.worker.scheduler.envVars != _|_ {
							env: list.Concat([#config._globalEnv, #config.worker.scheduler.envVars])
						}
						livenessProbe:  #config.worker.scheduler._resolvedLivenessProbe
						readinessProbe: #config.worker.scheduler._resolvedReadinessProbe
						volumeMounts: [
							{
						...
						resources:       #config.worker.scheduler.resources
