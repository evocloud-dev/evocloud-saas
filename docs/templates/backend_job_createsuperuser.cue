...
#CreateSuperuserJob: batchv1.#Job & {
	#config: #Config
	...
	kind:       "Job"
	metadata: {
		name:      "\(#config.metadata.name)-createsuperuser"
		namespace: #config.metadata.namespace
		labels:    #config.metadata.labels & {
			"app.kubernetes.io/component": "backend-createsuperuser"
		}
		annotations: {
	...
	spec: batchv1.#JobSpec & {
		...
		template: {
			...
			spec: corev1.#PodSpec & {
				...
				restartPolicy:         backend.createsuperuser.restartPolicy
				...
				containers: [
					{
						...
						if backend.createsuperuser.command != _|_ { command: backend.createsuperuser.command }
