		...
		template: spec: corev1.#PodSpec & {
			containers: [{
				...
				command: [
					"/bin/sh",
					"-c",
					"/usr/bin/mc alias set docs http://\(#config.metadata.name)-minio:9000 dinum password && (/usr/bin/mc ls docs/docs-media-storage >/dev/null 2>&1 || /usr/bin/mc mb docs/docs-media-storage) && exit 0",
				]
			}]
			restartPolicy: "Never"
		}
