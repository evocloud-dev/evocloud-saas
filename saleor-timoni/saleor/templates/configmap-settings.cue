...
#SettingsConfigMap: corev1.#ConfigMap & {
	...
	kind:       "ConfigMap"
	metadata: {
		name:      "\(#config.metadata.name)-settings"
...
def SENTRY_INIT(dsn: str, sentry_opts: dict):
    ...
    SALEOR_PII_DENYLIST = DEFAULT_PII_DENYLIST + [\"private_metadata\"]
    sentry_opts[\"event_scrubber\"] = EventScrubber(
        denylist=SALEOR_DENYLIST, pii_denylist=SALEOR_PII_DENYLIST
    )
    sentry_sdk.init(dsn=dsn, **sentry_opts)


if SENTRY_DSN:
    SENTRY_INIT(SENTRY_DSN, SENTRY_OPTS)
"""
	}
}
