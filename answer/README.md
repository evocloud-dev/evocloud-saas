...
A [timoni.sh](http://timoni.sh) module for deploying answer to Kubernetes clusters.
...
```shell
timoni -n default apply answer oci://<container-registry-url>
```
...
### Direct Apply (Local Development)

To apply directly from your local module directory:

```shell
timoni apply -n <namespace> <instance-name> .
```

### Apply with Runtime Secret Injection

To deploy while passing sensitive passwords dynamically via standard input (without committing secrets to disk):
...
}' | timoni apply -n answer answer . -f -
