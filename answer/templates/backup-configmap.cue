...
#BackupConfigMap: {
	...
	data: {
		"sqlite-backup.sh": """
			...
			archive="/backup/out/${BACKUP_ARCHIVE_PREFIX}-sqlite-${timestamp}.tar.gz"
			mkdir -p /backup/out
		...
		"postgres-backup.sh": """
			...
			pg_dump ${PG_DUMP_EXTRA_ARGS:-} --host="${DB_HOST}" --port="${DB_PORT}" --username="${DB_USERNAME}" --dbname="${DB_NAME}" | gzip -c > "${archive}"
			printf "%s" "${archive}" > /backup/out/backup-file
			"""
		"mysql-backup.sh": """
			#!/bin/sh
			...
			mysqldump ${MYSQL_DUMP_EXTRA_ARGS:-} --host="${DB_HOST}" --port="${DB_PORT}" --user="${DB_USERNAME}" "${DB_NAME}" | gzip -c > "${archive}"
			printf "%s" "${archive}" > /backup/out/backup-file
			"""
		"upload-backup.sh": """
			#!/bin/sh
