		...
		template: {
			...
			spec: corev1.#PodSpec & {
				...
				initContainers: [{
	                ...
	                image: "busybox:1.36"
	                command: ["sh", "-c", "chmod 1777 /tmp && mkdir -p \(#config.persistence.data.mountPath)/docuseal && chown -R 1000:1000 \(#config.persistence.data.mountPath) && chmod -R u+rwX,g+rwX \(#config.persistence.data.mountPath) && ls -laR \(#config.persistence.data.mountPath)"]
	                securityContext: {
						runAsUser: 0
	                }
                	volumeMounts: [
