...
#PostgresqlReadExtendedConfigMap: corev1.#ConfigMap & {
	#config: #Config
	...
	kind:       "ConfigMap"
	metadata: {
		name:      "\(#config.metadata.name)-postgresql-read-extended-configuration"
...
#PostgresqlReadMetricsConfigMap: corev1.#ConfigMap & {
	#config: #Config
	let pg = #config."hyperswitch-app".postgresql
	let globalAnn = *#config.metadata.annotations | {}
	apiVersion: "v1"
	kind:       "ConfigMap"
	metadata: {
		name:      "\(#config.metadata.name)-postgresql-read-metrics"
		namespace: #config.metadata.namespace
		labels: {
			for k, v in #config.metadata.labels if k != "app.kubernetes.io/name" {"\(k)": v}
...
#PostgresqlReadMetricsService: corev1.#Service & {
	#config: #Config
	let pg = #config."hyperswitch-app".postgresql
	let globalAnn = *#config.metadata.annotations | {}
	apiVersion: "v1"
...
#PostgresqlReadNetworkPolicy: netv1.#NetworkPolicy & {
	#config: #Config
	let pg = #config."hyperswitch-app".postgresql
	let globalAnn = *#config.metadata.annotations | {}
	...
	kind:       "NetworkPolicy"
	metadata: {
		name:      "\(#config.metadata.name)-postgresql-read"
		namespace: #config.metadata.namespace
		labels: {
			for k, v in #config.metadata.labels if k != "app.kubernetes.io/name" {"\(k)": v}
			"app.kubernetes.io/component": "read"
		...
		ingress: [
			{
				...
				if !pg.readReplicas.networkPolicy.allowExternal {
					from: [
						...
						{
							podSelector: matchLabels: {
								"\(#config.metadata.name)-postgresql-read-client": "true"
							}
						},
...
#PostgresqlReadPDB: policyv1.#PodDisruptionBudget & {
	#config: #Config
	let pg = #config."hyperswitch-app".postgresql
	let globalAnn = *#config.metadata.annotations | {}
...
#PostgresqlReadServiceMonitor: {
	...
	metadata: {
		name:      "\(#config.metadata.name)-postgresql-read"
		namespace: pg.metrics.serviceMonitor.namespace | #config.metadata.namespace
		labels: {
			for k, v in #config.metadata.labels if k != "app.kubernetes.io/name" {"\(k)": v}
			"app.kubernetes.io/component": "metrics-read"
			"app.kubernetes.io/name":      "postgresql"
...
#PostgresqlReadStatefulSet: appsv1.#StatefulSet & {
	#config: #Config
	let pg = #config."hyperswitch-app".postgresql
	let globalAnn = *#config.metadata.annotations | {}
	apiVersion: "apps/v1"
	...
	spec: {
		...
		serviceName: "\(#config.metadata.name)-postgresql-read-headless"
		...
		template: {
			...
			spec: {
				serviceAccountName: [if pg.serviceAccount.name != "" {pg.serviceAccount.name}, "\(#config.metadata.name)-postgresql"][0]
				...
				if pg.readReplicas.terminationGracePeriodSeconds != "" {
				...
				initContainers: [
					{
						...
						image:           "\(pg.volumePermissions.image.registry)/\(pg.volumePermissions.image.repository):\(pg.volumePermissions.image.tag)"
					...
					if pg.readReplicas.initContainers != [] {
				...
				containers: [
					{
						...
						env: [
							...
							{name: "POSTGRES_MASTER_HOST", value: "\(#config.metadata.name)-postgresql"},
					...
					if pg.readReplicas.extraContainers != [] {
				...
				volumes: [
					if pg.readReplicas.extendedConfiguration != "" {
						{
							name: "postgresql-extended-config"
							configMap: {name: "\(#config.metadata.name)-postgresql-read-extended-configuration"}
						}
					},
...
#PostgresqlReadService: corev1.#Service & {
	#config: #Config
	let pg = #config."hyperswitch-app".postgresql
	let globalAnn = *#config.metadata.annotations | {}
	apiVersion: "v1"
...
#PostgresqlReadHeadlessService: corev1.#Service & {
	#config: #Config
	let pg = #config."hyperswitch-app".postgresql
	let globalAnn = *#config.metadata.annotations | {}
	apiVersion: "v1"
	kind:       "Service"
	metadata: {
		name:      "\(#config.metadata.name)-postgresql-read-headless"
		namespace: #config.metadata.namespace
		labels: {
			for k, v in #config.metadata.labels if k != "app.kubernetes.io/name" {"\(k)": v}
			"app.kubernetes.io/component": "read"
