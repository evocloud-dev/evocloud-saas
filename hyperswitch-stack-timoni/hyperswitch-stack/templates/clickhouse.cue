...
#ClickhouseConfigMap: corev1.#ConfigMap & {
	#config: #Config

	apiVersion: "v1"
	kind:       "ConfigMap"
	metadata: {
		name:      "\(#config.metadata.name)-\(#config."hyperswitch-app".clickhouse.name)-script"
	...
	data: "init.sql": """
		  init.sql: |-
		    ...
		    ) ENGINE = Kafka SETTINGS kafka_broker_list = '\(#config.metadata.name)-kafka:9092',
