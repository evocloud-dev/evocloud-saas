		...
		template: {
			...
			spec: corev1.#PodSpec & {
				...
				containers: [{
					command: ["/bin/sh", "-c"]
					args: [
						"mkdir -p /tmp/rails-tmp && chmod 700 /tmp/rails-tmp && export TMPDIR=/tmp/rails-tmp && exec docker/entrypoints/rails.sh bundle exec rails s -p \(#config.services.internalPort) -b 0.0.0.0",
					]
					env: [
