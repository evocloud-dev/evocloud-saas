...
#CustomConfigMap: corev1.#ConfigMap & {
	...
	metadata: {
		...
		annotations: {
			...
			createdFrom: #configVal.localPath
		}
	}
	data: {
		"\(#name)": #content
	...
	#content: {
		if #configVal.localPath == "pimcore-config/skeleton/config.yaml" {
			#defaultConfigYaml
		}
		if #configVal.localPath == "pimcore-config/skeleton/bundles.php" {
			#defaultBundlesPhp
		}
		if #configVal.localPath != "pimcore-config/skeleton/config.yaml" && #configVal.localPath != "pimcore-config/skeleton/bundles.php" {
			#configVal.content
		}
	}
	...
	#defaultConfigYaml: """
		...
		# or even more specific:
		# ./bin/console debug:config pimcore assets.image.thumbnails
		    ...
		    cache:
		       pools:
		           pimcore.cache.pool:
		               ...
		               provider: '%env(REDIS_DSN)%'  # Redis DNS, see: https://symfony.com/doc/current/components/cache/adapters/redis_adapter.html#configure-the-connection
...
#MountedConfigMap: corev1.#ConfigMap & {
	#config:    #Config
	#name:      string
	#configVal: _
