...
#ConfigMapMaintenanceCronjobEnv: corev1.#ConfigMap & {
	#config: #Config

	apiVersion: "v1"
	kind:       "ConfigMap"
	metadata: {
		name:      "\(#config.metadata.name)-maintenance-cronjob-env"
	...
	data: {
		PHP_MEMORY_LIMIT:              #config.php.ini.maintenance.cronjob.phpMemoryLimit
		PHP_MAX_EXECUTION_TIME:        "\(#config.php.ini.maintenance.cronjob.phpMaxExecutionTime)"
