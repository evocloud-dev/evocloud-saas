...
#NginxConfigTpl: {
	...
	let _ipFamilies = #in.nginx.ipFamilies | *[]
	let _ipv4Enabled = len([for ip in _ipFamilies if ip == "IPv4" {ip}]) > 0
	let _ipv6Enabled = len([for ip in _ipFamilies if ip == "IPv6" {ip}]) > 0

	let _listenDirective = [
		if _ipv4Enabled && _ipv6Enabled {
			"""
			    # Both IPv4 and IPv6 are enabled
			    listen [::]:\(#in.nginx.containerPort) ipv6only=off;
			"""
		},
		if !_ipv4Enabled && _ipv6Enabled {
			"""
			    # Only IPv6 is enabled
			    listen [::]:\(#in.nginx.containerPort);
			"""
		},
		if _ipv4Enabled && !_ipv6Enabled {
			"""
	...
	let _headers = [
		for k, v in #in.nginx.config.headers if v != "" {
			"    add_header \(k) \"\(v)\" always;"
		},
	]

	let _serverBlockCustom = #in.nginx.config.serverBlockCustom
	...
	out: """
		...
		\(strings.Join(_headers, "\n"))

		\(strings.Join([for line in strings.Split(_serverBlockCustom, "\n") { "    " + line }], "\n"))
