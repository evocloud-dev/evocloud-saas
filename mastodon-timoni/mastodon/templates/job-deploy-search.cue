...
#JobDeploySearch: batchv1.#Job & {
	...
	metadata: {
		...
		name:   "\(#config.metadata.name)-deploy-search"
		...
		template: {
			...
			spec: corev1.#PodSpec & {
				...
				containers: [{
					...
					args: [
						if #config.mastodon.hooks.deploySearch.concurrency != _|_ { "--concurrency" },
						if #config.mastodon.hooks.deploySearch.concurrency != _|_ { "\(#config.mastodon.hooks.deploySearch.concurrency)" },
						if #config.mastodon.hooks.deploySearch.resetChewy { "--reset-chewy" },
						if #config.mastodon.hooks.deploySearch.only != "" { "--only" },
						if #config.mastodon.hooks.deploySearch.only != "" { #config.mastodon.hooks.deploySearch.only },
					]
					envFrom: [
					...
					resources: #config.mastodon.hooks.deploySearch.resources
				}]
				nodeSelector: #config.mastodon.hooks.deploySearch.nodeSelector | #config.nodeSelector
			}
		}
	}
}
