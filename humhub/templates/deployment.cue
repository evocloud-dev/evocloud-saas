		...
		template: {
			metadata: {
				labels: #config.metadata.labels & {
					...
					"truecharts.org/pvc": "assets_config_modules_themes_uploads"
				}
			}
			spec: corev1.#PodSpec & {
						...
						labelSelector: matchExpressions: [
							{
								...
								operator: "In"
								values: ["assets_config_modules_themes_uploads"]
							},
						]
					},
				]
				...
				containers: [
					{
						...
						ports: [
							{
								name:          "backend"
								containerPort: #config.service.backend.ports.backend.port
								protocol:      "TCP"
							},
						]
						volumeMounts: [
						...
						livenessProbe: {
							exec: command: [#config.workload.main.podSpec.containers.main.probes.liveness.command]
						...
						readinessProbe: {
							exec: command: [#config.workload.main.podSpec.containers.main.probes.readiness.command]
						...
						startupProbe: {
							exec: command: [#config.workload.main.podSpec.containers.main.probes.startup.command]
							initialDelaySeconds: 10
							periodSeconds:       5
							failureThreshold:    60
							successThreshold:    1
						...
						securityContext: {
							...
							capabilities: {
								add:  #config.mainCapabilities.add
								drop: #config.mainCapabilities.drop
							}
						}
						env: [
							...
							{name: "HUMHUB_ADMIN_EMAIL", value: #config.humhub.admin.email},
							{name: "HUMHUB_ADMIN_LOGIN", value: #config.humhub.admin.login},
							{name: "HUMHUB_ADMIN_PASSWORD", value: #config.humhub.admin.password},
							{name: "HUMHUB_AUTO_INSTALL", value: "true"},
							{name: "HUMHUB_CACHE_CLASS", value: "yii\\redis\\Cache"},
							{name: "HUMHUB_CACHE_EXPIRE_TIME", value: "3600"},
							{
								name: "HUMHUB_DB_HOST"
							...
							{name: "HUMHUB_DB_NAME", value: #config.mariadb.mariadbDatabase},
							{name: "HUMHUB_DB_PASSWORD", value: #config.mariadb.password},
							{name: "HUMHUB_DB_USER", value: #config.mariadb.mariadbUsername},
							{name: "HUMHUB_HOST", value: #config.humhub.host},
							{name: "HUMHUB_INTEGRITY_CHECK", value: "true"},
							{name: "HUMHUB_MAILER_ALLOW_SELF_SIGNED_CERTS", value: "\(#config.humhub.mailer.allow_self_sign_certs)"},
							{name: "HUMHUB_MAILER_ENCRYPTION", value: #config.humhub.mailer.encrypt},
							{name: "HUMHUB_MAILER_HOSTNAME", value: #config.humhub.mailer.hostname},
							{name: "HUMHUB_MAILER_PASSWORD", value: #config.humhub.mailer.password},
							{name: "HUMHUB_MAILER_PORT", value: "\(#config.humhub.mailer.port)"},
							{name: "HUMHUB_MAILER_SYSTEM_EMAIL_ADDRESS", value: #config.humhub.mailer.sys_address},
							{name: "HUMHUB_MAILER_SYSTEM_EMAIL_NAME", value: #config.humhub.mailer.sys_name},
							{name: "HUMHUB_MAILER_TRANSPORT_TYPE", value: #config.humhub.mailer.type},
							{name: "HUMHUB_MAILER_USERNAME", value: #config.humhub.mailer.user},
							{name: "HUMHUB_PROTO", value: #config.humhub.proto},
							{name: "HUMHUB_QUEUE_CLASS", value: "humhub\\modules\\queue\\driver\\Redis"},
							{
							...
							{name: "HUMHUB_REDIS_PASSWORD", value: #config.redis.password},
							{name: "HUMHUB_REDIS_PORT", value: "\(#config.redis.port)"},
				...
				volumes: [
					...
					if #config.persistence.config.enabled {
						{
							name: "config"
							persistentVolumeClaim: claimName: #config.persistence.config.name
						}
					},
					if #config.persistence.assets.enabled {
						{
