...
#MergeDuplicateUsersJob: batchv1.#Job & {
	#config: #Config
	...
	kind:       "Job"
	metadata: {
		name:      "\(#config.metadata.name)-merge-duplicate-users"
		namespace: #config.metadata.namespace
		labels:    #config.metadata.labels & {
			"app.kubernetes.io/component": "backend-merge-duplicate-users"
		}
		annotations: {
			"argocd.argoproj.io/sync-options": "Replace=true,Force=true"
			if backend.mergeDuplicateUsers.annotations != _|_ {
				backend.mergeDuplicateUsers.annotations
			}
		}
	}
	spec: batchv1.#JobSpec & {
		...
		template: {
			...
			spec: corev1.#PodSpec & {
				...
				restartPolicy:         backend.mergeDuplicateUsers.restartPolicy
				...
				containers: [
					{
						...
						if backend.mergeDuplicateUsers.command != _|_ { command: backend.mergeDuplicateUsers.command }
